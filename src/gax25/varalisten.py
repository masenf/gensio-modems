#!/usr/bin/python3
"""
varalisten - listen for vara connections to hand off to a program or console
"""
...
def main():
    ...
    parser.add_argument("-l", "--listen", help="SSID to listen on")
    parser.add_argument(
        "-p1",
        "--vara-command-port",
        default="tcp,localhost,8300",
        help="gensio vara command connection string",
    )
    parser.add_argument(
        "-p2",
        "--vara-data-port",
        default="tcp,localhost,8301",
        help="gensio vara data connection string",
    )
    parser.add_argument(
        "--stderr",
        action="store_true",
        default=False,
        help="Also send stderr over vara connection",
    )
